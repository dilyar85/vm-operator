...
func cdromTests() {
	...
	Context("UpdateCdromDeviceChanges", func() {
		...
		Context("Happy Path (no errors occurred)", func() {
			...
			When("VM.spec.Cdrom replaces existing CD-ROM devices", func() {
				...
				It("should remove the existing CD-ROM device and add a new CD-ROM device with the correct controller assigned", func() {
					Expect(result).To(HaveLen(2))
					// First result should be adding the new CD-ROM device with its unit number being 0 on the controller.
