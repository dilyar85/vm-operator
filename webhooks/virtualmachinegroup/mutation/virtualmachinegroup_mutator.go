...
func ProcessPowerState(
	...
	if vmGroup.Spec.PowerState != oldVMGroup.Spec.PowerState {
		ctx.Logger.V(4).Info("Power state change detected in VirtualMachineGroup",
			"oldPowerState", oldVMGroup.Spec.PowerState,
			"newPowerState", vmGroup.Spec.PowerState)
		...
		vmGroup.Annotations[vmopv1.LastUpdatedPowerStateTimeAnnotation] = now

		ctx.Logger.V(4).Info("Set last updated power state time annotation", "time", now)
		return true
	}
