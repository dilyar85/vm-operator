...
func intgTestsValidateCreate() {
	...
	DescribeTable("create table", validateCreate,
		...
		Entry("should not work with invalid power op member",
			createArgs{invalidPowerOpMember: true}, false, memberNotFoundInGroup),
		Entry("should not work with invalid time format",
			createArgs{invalidTimeFormat: true}, false, invalidTimeFormat),
	)
}
...
func intgTestsValidateUpdate() {
	...
	When("update is performed with empty power state after it's been set", func() {
		...
		It("should deny the request", func() {
			...
			Expect(err.Error()).To(ContainSubstring(emptyPowerStateNotAllowedAfterSet))
		})
	})

	When("update is performed with invalid power op member", func() {
		BeforeEach(func() {
			ctx.vmGroup.Spec.PowerOnOp = []vmopv1.VirtualMachineGroupPowerOp{
				{
					Name: "non-existent-vm",
		...
		It("should deny the request", func() {
			Expect(err).To(HaveOccurred())
			expectedPath := field.NewPath("spec", "powerOnOp")
			Expect(err.Error()).To(ContainSubstring(expectedPath.String()))
			Expect(err.Error()).To(ContainSubstring(memberNotFoundInGroup))
		})
	})
	...
	When("update is performed with invalid time format in last-updated-power-state annotation", func() {
		...
		It("should deny the request", func() {
			Expect(err).To(HaveOccurred())
			expectedPath := field.NewPath("metadata", "annotations").Key(vmopv1.LastUpdatedPowerStateTimeAnnotation)
			Expect(err.Error()).To(ContainSubstring(expectedPath.String()))
			Expect(err.Error()).To(ContainSubstring(invalidTimeFormat))
		})
	})
